...
const ReasoningLab: React.FC<ReasoningLabProps> = ({ currentUser, evaluationName, onLeaveEvaluation }) => {
  // View mode state
  const [viewMode, setViewMode] = useState<'list' | 'dashboard'>('list');
  const [isLoadingEvaluations, setIsLoadingEvaluations] = useState<boolean>(true);
  const [showAllEvaluations, setShowAllEvaluations] = useState<boolean>(false);
  ...
  const isAdminShowAll = currentUser.role === 'admin' && showAllEvaluations;

  const visibleEvaluations = allEvaluations.filter(evaluation => {
    // Filter by named evaluation session unless admin is viewing all
    if (!isAdminShowAll && evaluation.evaluationName !== evaluationName) return false;
  ...
  // For admin "show all" mode: group evaluations by their named session
  const groupedEvaluations: Record<string, ReasoningEvaluationRecord[]> = isAdminShowAll
    ? visibleEvaluations.slice().reverse().reduce((acc, ev) => {
        const key = ev.evaluationName || 'Unnamed';
        if (!acc[key]) acc[key] = [];
        acc[key].push(ev);
        return acc;
      }, {} as Record<string, ReasoningEvaluationRecord[]>)
    : {};
  
  return (
    <div className="space-y-16">
        {/* Evaluation session banner */}
        <div className="flex items-center justify-between px-5 py-3 bg-primary/5 border border-primary/20 rounded-xl">
          <div className="flex items-center gap-3">
            <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Evaluation</span>
            <span className="font-bold text-foreground text-base">{evaluationName}</span>
          </div>
          <button
            onClick={onLeaveEvaluation}
        ...
        <section>
           <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-8 pb-4 border-b border-border/70">
            <div>
              <h2 className="text-xl sm:text-2xl font-bold text-foreground">
                {isAdminShowAll ? 'All Comparison Reports' : `"${evaluationName}" Reports`}
              </h2>
              {currentUser.role === 'admin' && (
                ...
                >
                  {isAdminShowAll ? `Show only "${evaluationName}"` : 'Show all evaluations (admin)'}
                </button>
              )}
            </div>
          ...
          {isLoadingEvaluations ? (
            ...
            <div className="text-center py-10 bg-card border border-border rounded-xl shadow-sm">
             ...
             viewMode === 'list' ? (
                isAdminShowAll ? (
                  // Grouped by evaluation name for admin "show all" view
                  <div className="space-y-12">
                    {Object.entries(groupedEvaluations).map(([groupName, groupEvals]) => (
                      <div key={groupName}>
                                ...
                                <div className="px-6 py-6 border-t border-border bg-background/50 text-sm space-y-6">
                                  ...
                                  <div className="mt-6 flex justify-between items-center pt-4 border-t border-border/50">
                                       <div className="flex items-center gap-2">
                                          {canEdit && (
                                              <button onClick={() => handleStartEdit(ev.id)} className="px-4 py-2 text-xs font-semibold rounded-lg transition-colors flex items-center gap-2 text-primary hover:bg-primary/10" aria-label="Edit this evaluation">
                                          ...
                                          <button onClick={() => handleDeleteEvaluation(ev.id)} className="px-4 py-2 text-xs font-semibold rounded-lg transition-colors flex items-center gap-2 text-destructive hover:bg-destructive/10" aria-label="Delete this evaluation">
